...
import { User, Camera, Save, Fingerprint, Send } from 'lucide-react';
  ...
  const handleAvatarChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        // Beritahu sidebar agar langsung memperbarui foto
            ...
            {(user?.role === 'guru' || user?.role === 'wali_murid') && (
              <div>
                ...
                <div className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-3 flex justify-between items-center">
                  ...
                  <button onClick={handleRegisterFingerprint} className={`text-xs font-bold hover:underline px-3 py-1.5 rounded-lg border transition-colors ${
                    user?.has_fingerprint
                      ? 'text-blue-700 dark:text-blue-400 bg-blue-100 dark:bg-blue-900/30 border-blue-200 dark:border-blue-800'
                      : 'text-green-700 dark:text-green-400 bg-green-100 dark:bg-green-900/30 border-green-200 dark:border-green-800'
                  }`}>
                    {user?.has_fingerprint ? 'PERBARUI' : 'DAFTARKAN'}
                  </button>
                </div>
              </div>
            )}
