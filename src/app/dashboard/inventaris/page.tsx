      ...
      <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
        {dorms.map(dorm => {
          if (user?.role === 'pengurus_asrama') {
             // Pengurus asrama hanya melihat tab asramanya sendiri
             const myDorm = (user?.nama || '').match(/asrama\s+([a-z])/i)?.[1]?.toUpperCase();
             if (myDorm && dorm !== 'Semua' && dorm !== myDorm) return null;
          }
          return (
          <button
            key={dorm}
            onClick={() => setActiveTab(dorm)}
            className={`flex items-center gap-2 px-5 py-2.5 rounded-2xl font-bold whitespace-nowrap transition-all border ${
              activeTab === dorm 
                ? 'bg-indigo-600 border-indigo-600 text-white shadow-md' 
                : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-500 hover:bg-indigo-50 dark:hover:bg-gray-700'
            }`}
          >
            {dorm === 'Semua' ? <Archive size={16}/> : <MapPin size={16}/>}
            {dorm === 'Semua' ? 'Semua Asrama' : dorm === 'Tahfid' ? 'Tahfid' : `Asrama ${dorm}`}
          </button>
        )})}
      </div>
