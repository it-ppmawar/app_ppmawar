        ...
        <>
          ...
          {tipeAda.size > 1 && (
            <div className="flex flex-col gap-2 w-full">
              {/* Row 1: Semua (Full Width) */}
              <div className="w-full bg-gray-100 dark:bg-gray-800 p-1.5 rounded-2xl border border-gray-200 dark:border-gray-700">
                <button
                ...
                >
                  ...
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-extrabold ${
                    filter === 'semua' ? 'bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300' : 'bg-gray-200 dark:bg-gray-700 text-gray-500'
                  }`}>{schedules.length}</span>
                </button>
              </div>

              {/* Row 2: Tabs Lainnya (Berdampingan, Scrollable jika tidak muat) */}
              <div className="flex gap-2 p-1.5 bg-gray-100 dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 overflow-x-auto scrollbar-none">
                {(['quran', 'madin', 'kegiatan'] as TipeFilter[]).filter(t => tipeAda.has(t)).map(t => (
                  <button
                    key={t}
                    onClick={() => setFilter(t)}
                    className={`flex items-center justify-center gap-1.5 px-4 py-2.5 text-sm font-bold rounded-xl transition-all whitespace-nowrap flex-1 min-w-[120px] ${
                      filter === t
                        ? 'bg-white dark:bg-gray-700 text-blue-700 dark:text-blue-300 shadow-sm'
                        : 'text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:hover:text-gray-300'
                    }`}
                  >
                    {tipeLabel[t]}
                    <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-extrabold ${
                      filter === t ? 'bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300' : 'bg-gray-200 dark:bg-gray-700 text-gray-500'
                    }`}>{countByTipe(t)}</span>
                  </button>
                ))}
              </div>
