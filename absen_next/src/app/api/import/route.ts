...
async function importBilling(workbook: XLSX.WorkBook) {
  ...
  for (const sheetName of workbook.SheetNames) {
    ...
    for (let i = 0; i < dataRows.length; i++) {
      ...
      if (totalTunggakan <= 0 && syahriyah <= 0 && jariyah <= 0 && daftarUlang <= 0) {
        result.skipped++;
        continue;
      }
      ...
      try {
        for (const bill of billings) {
          if (bill.amount > 0) {
            ...
            await pool.execute(
              `INSERT INTO billing (nis, nama_santri, asrama, kamar, nama_tagihan, nominal, status, periode, source)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'excel')
               ON DUPLICATE KEY UPDATE
                 ...
                 source = 'excel'`,
              [nis, nama, asrama, kamar || '-', bill.name, bill.amount, status, periode]
            );
          } else {
            await pool.execute(
              `DELETE FROM billing WHERE nis = ? AND nama_tagihan = ? AND periode = ? AND status = 'Belum'`,
              [nis, bill.name, '2026-2027']
            );
          }
        }
        result.inserted++;
